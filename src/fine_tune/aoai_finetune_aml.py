...
def main():
    ...
    parser.add_argument("--training_file_name", type=str, default=None)
    parser.add_argument("--validation_file_name", type=str, default=None)
    parser.add_argument("--test_file_dir", type=str, default=None)
    parser.add_argument("--api_key", type=str)
    parser.add_argument("--api_base", type=str, default=None)
    parser.add_argument("--finetune_job_id", type=str, default=None)
    ...
    parser.add_argument("--check_interval", type=int, default=5 * 60, help="Time interval (in seconds) to check job status")
    # finetune hp
    parser.add_argument("--model", type=str)
    parser.add_argument("--n_epochs", type=int, default=None)
    parser.add_argument("--learning_rate_multiplier", type=float, default=None)
    ...
    if args.deployment_id:
        logger.info(f"detected non-empty deployment id: {args.deployment_id}, will skip finetune-deployment part")
        curr_deployment_id = args.deployment_id
    else:
        if args.finetune_job_id:
            logger.info(f"detected non-empty finetune id: {args.finetune_job_id}, will skip finetune part")
            finetune_hp = {}
        else:
    ...
    if args.model in NL2SQL_SPIDER_MODELS:
        AOAI_inferencer = AOAIInferencer_nl2sql_spider()
        if args.truncate_txt:
            logger.info(f"args.truncate_txt is True while benchmarking {args.model}, changing to False")
            args.truncate_txt = False
    else:
        AOAI_inferencer = AOAIInferencer()
    AOAI_inferencer.inference_dir(args=args,
                                  input_dir=args.test_file_dir,
                                  output_file_path=os.path.join(args.output_dir, OUTPUT_FILE_NAME),
                                  deployment_id=curr_deployment_id,
                                  max_tokens=args.max_tokens,
                                  print_interval=args.print_interval,
                                  )
    logger.info(f"AOAI_inferencer.total_inference_calls: {AOAI_inferencer.total_inference_calls}, AOAI_inferencer.total_successful_inference_calls: {AOAI_inferencer.total_successful_inference_calls}, AOAI_inferencer.total_erroneous_inference_calls: {AOAI_inferencer.total_erroneous_inference_calls}")
