...
parser.add_argument("--raw_data", default="./data/1raw")
parser.add_argument("--train_output", default="./data/2train")
parser.add_argument("--validation_output", default="./data/2validation")
...
filepaths = [f for f in listdir(args.raw_data) if f.endswith('.csv')]

for file in filepaths:
  print(f"processing {file}")
  df = pd.read_csv(args.raw_data + '/' + file)
  ## pick columns
  ...
  train.to_json(f"{args.train_output}/{file}.jsonl", orient='records', lines=True)
  print(f"wrote train {file}.jsonl")
  validation.to_json(f"{args.validation_output}/{file}.jsonl", orient='records', lines=True)
  print(f"wrote validation {file}.jsonl")
